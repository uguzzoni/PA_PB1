...
# NO:   0  gen_energy_A        : ew low   (1a=0.02 1b=0.05 2=0.02 3=0.05)  → best mean
...
# NO:   2  opt_hard_energy_C   : stage 3 only,          ew3=0.50,  n=1/seq  → fast screen
#    3  opt_anneal_noenergy : stage 2+3,              no energy, n=4/seq  → structural refinement
# NO:   4  opt_anneal_energy_B : stage 2+3, ew2=0.02,   ew3=0.20,  n=4/seq  → medium energy
...
SEQ_FILE=${SCRIPT_DIR}/prg/PA_PB1/data/seqs_aff_alberto_darren.txt  # alberto-generated sequences
...
# ── Run protocol ──────────────────────────────────────────────────────────────
cd ${SCRIPT_DIR}
OUTPUT=${OUTDIR}/${PROTO}_albertoseqs.json
...
case "$PROTO" in
    ...
    opt_anneal_energy_B)
        python3 $OPT_SCRIPT --seq_file $SEQ_FILE \
            --ew_2 0.02 --ew_3 0.20 -n 5 \
            -o $OUTPUT
        ;;

    opt_anneal_energy_C)
        python3 $OPT_SCRIPT --seq_file $SEQ_FILE \
            --ew_2 0.05 --ew_3 0.50 -n 5 \
            -o $OUTPUT
        ;;
