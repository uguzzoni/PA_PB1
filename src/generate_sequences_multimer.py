"""
generate_sequences_multimer.py

Same 4-stage energy-guided protocol as generate_sequences.py, but uses
AF2-Multimer (use_multimer=True) instead of AF2-pTM.
...
Model weights loaded: params_model_{1..5}_multimer_v3.npz (whichever exist in PARAMS_DIR).
Loss function: standard colabdesign binder loss (i_pae, i_con, plddt, ...) + energy MLP.

Usage:
    python generate_sequences_multimer.py                        # all defaults
    python generate_sequences_multimer.py -o results.json        # custom output file
    python generate_sequences_multimer.py -n 50 --ew_1a 0.0 --ew_1b 0.0 --ew_2 0.0 --ew_3 0.0
    python generate_sequences_multimer.py --ew_1a 0.02 --ew_1b 0.2 --ew_2 0.05 --ew_3 0.2
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate binder sequences with AF2-Multimer and per-stage energy weights.")
    parser.add_argument("-o", "--output", default=None,
                        help="Output JSON file (default: generated_sequences_multimer_ew<1a>_<1b>_<2>_<3>.json)")
    ...
    out_file = args.output or os.path.join(
        OUT_DIR, f"generated_sequences_multimer_ew{EW_1A:.2f}_{EW_1B:.2f}_{EW_2:.2f}_{EW_3:.2f}.json"
    )
    ...
    print(f"Models loaded: {model._model_names}")
